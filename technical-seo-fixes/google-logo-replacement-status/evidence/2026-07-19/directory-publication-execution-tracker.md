...
| DIR-014 | Nextdoor Business | Published, logo upload pending | On July 24, the account email was verified and authenticated dashboard access was confirmed. The public page at `https://nextdoor.com/pages/life-for-relief-and-development-southfield-mi/` shows the correct legal name, address, phone, public email, description, and canonical HTTPS backlink. The editor exposes a photo replacement control, but the approved logo upload is waiting for Chrome extension file access to be enabled. The current generic Nextdoor storefront image means the profile does not count yet | No |
...
| DIR-017 | Taproot Plus | Organization approved, logo upload pending | Google returned no exact organization result. On July 24, the authorized account email was verified, nonprofit onboarding was completed with the legal name, EIN `95-4402149`, canonical website, mission, location, and issue areas, and Taproot displayed “Congratulations! You're officially a Taproot nonprofit!” The dashboard marks Life for Relief and Development as `Approved` and exposes an organization-profile upload control. The approved logo upload and logged-out public backlink verification remain pending | No |
...
| NEW-013 | Nextdoor Business | Local organization profile | Published, logo upload pending | Email verification and authenticated editor access are complete. The public page has the correct details and canonical HTTPS backlink; the approved logo upload is waiting for Chrome extension file access |
...
| NEW-022 | Taproot Plus | Skilled-volunteer nonprofit profile | Organization approved, logo upload pending | Email verification and nonprofit onboarding are complete. Taproot marks the organization `Approved`; upload the approved logo and verify the public profile and backlink |
...
- Taproot account verification and nonprofit onboarding are complete. The organization is approved, but the logo upload and logged-out profile verification remain.
...
- LaCartes and 2FindLocal are staged at their required human-verification steps.
...
- Nextdoor is published with the correct public details and HTTPS backlink. Its editor is accessible, but the approved logo upload is waiting for Chrome extension file access.
