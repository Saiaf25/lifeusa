...
Checked July 26, 2026 from 10:00 to 10:12 Europe/Istanbul. This was the second scheduled weekly check under automation `lifeusa-logo-signal-monitor`.
...
- Candid/GuideStar, GreatNonprofits, LinkedIn, both Facebook pages, Instagram, YouTube, X, and the Google Maps contributor identity show the approved blue-globe variant. Linktree shows the preferred yellow-ray avatar.
- The English Linktree Facebook button still routes to the Somali Facebook page. ReliefWeb still exposes no LifeUSA logo and still links to `http://www.lifeusa.org/`.
- Google Search and Images returned Google's anti-automation page for the required English, United States, personalization-disabled queries. Paid-result logo, organic favicon, Knowledge Panel state, and ranked Images state are therefore inconclusive for July 26.
- The logged-out Southfield Google Maps listing remained observable with 4.3 stars, 20 reviews, the correct address, the stale HTTP website link, and approved current-logo media.
...
A broader domain search surfaced a July 21 internal reply from Dr. Hany Saqr stating that LifeUSA has no record of who contacted the legacy pages. That message is an ownership-history blocker, not a publisher reply to SENT-001 or SENT-002, and it does not change either sent record.

## Retired-source comparison

| Source | Public reference on July 26 | Direct file | Comparison with July 15 evidence | Result |
|---|---|---|---|---|
...
The current WordPress image redirects from the documented files-domain URL to the equivalent WordPress uploads URL. The redirect does not represent a source correction because the page reference and decoded image remain unchanged.

## Acceptable authority-profile state

| Surface | Logged-out July 26 result |
|---|---|
...
| Candid / GuideStar | Page still embeds `https://docs.candid.org/edoc/11078205`; the 1500 by 1500 PNG retains SHA-256 `f440247655ec4a13196be3ee40b291c361de42e596a05941395fc22dd42b6328` and decoded-pixel absolute error `0` |
| LinkedIn | Public company page exposed a 200 by 200 blue-globe avatar and still lists the website as HTTP |
| Facebook, English | Logged-out page exposed the approved blue-globe avatar and current organization identity |
| Facebook, Somali | Logged-out page exposed the approved blue-globe avatar; this remains the incorrect destination used by the English Linktree |
| Instagram | Logged-out Open Graph data exposed a 100 by 100 blue-globe avatar, resolving the July 19 inconclusive state |
...
| Linktree | Public page exposed the preferred yellow-ray avatar and canonical HTTPS LifeUSA link, but its English Facebook button still resolves to profile ID `61563961181765` |
| ReliefWeb | Public profile returned `200`, exposed no LifeUSA logo, and still linked to `http://www.lifeusa.org/` |

Social CDN URLs are temporary and several rotated since July 15. The visible logo-family classification, not URL continuity alone, is the correctness test for these approved social variants.

## Owned signal and canonical asset health

| Asset or signal | July 26 result |
|---|---|
...
| Visible owned placements | Header and footer still reference the approved horizontal source and expose two occurrences of `Life for Relief and Development logo` alt text |
| Favicon | Public `icon`, `shortcut icon`, and `apple-touch-icon` links still derive from approved square source `6df904_3ba0b63af7e945f0a92fbe9978934569~mv2.png` |
...
| Profile | July 26 public result |
|---|---|
| Find-Us-Here | `200`; correct organization details, approved current square rendition, and canonical `https://www.lifeusa.org/` backlink remain visible |
...
| Nextdoor | `200`; logged-out page exposes the legal name, address, phone, public email, description, canonical backlink, and direct 1800 by 1800 logo whose decoded-pixel absolute error is `0` against the canonical square |
...
- photo gallery remained public and associated with contributor profile `115898584427846492369`;
- contributor avatar uses the approved blue-globe variant; and
- visible gallery media includes current yellow-ray LifeUSA branding.

This proves the current Maps state only. It does not prove the July 26 Search, paid-result, favicon, Knowledge Panel, or Images state.
...
Sunday, August 2, 2026 at 10:00 Europe/Istanbul.
